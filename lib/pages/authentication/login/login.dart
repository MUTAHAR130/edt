...
class _LoginScreenState extends State<LoginScreen> {
      ...
      if (user != null) {
        ...
        if (userRole == 'Passenger') {
          userExists = await _checkUserExistsInCollection(
              emailOrPhone, 'passengers', 'Passenger');
        } else if (userRole == 'Driver') {
          userExists = await _checkUserExistsInCollection(
              emailOrPhone, 'drivers', 'Driver');
        } else {
          EasyLoading.showError("Invalid Credentials");
          return;
        }
        String collectionName =
            userRole == 'Passenger' ? 'passengers' : 'drivers';
  ...
  Future<void> _updateUserToken(
      String email, String collectionName, String token) async {
    try {
  ...
  Future<bool> _checkUserExistsInCollection(
      String email, String collectionName, String roleField) async {
    try {
              ...
              children: [
                ...
                Row(
                  children: [
                    Expanded(
                        child:
                            Divider(endIndent: 10, color: Color(0xffB8B8B8))),
                    Text(
                      'or',
                      style: GoogleFonts.poppins(
